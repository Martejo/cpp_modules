...
void    Bureaucrat::decrementGrade()
{
	...
	std::cout << "Bureaucrat " << this->getName() <<  ": grade successfully decremented. " << std::endl;
}

void    Bureaucrat::incrementGrade()
{
	validateGrade(_grade - 1);
	std::cout << "Bureaucrat " << this->getName() <<  ": grade successfully incremented. " << std::endl;
}

std::ostream&   operator<<( std::ostream &os, const Bureaucrat &to_display )
{
	os  << to_display.getName() << ", Grade : " << to_display.getGrade();
	return(os);
}
